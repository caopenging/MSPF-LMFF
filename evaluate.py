...
parser.add_argument('--model', type=str, default=r'E:/6DPS/6D-CLGrasp/train_results/learningrate0.01', help='resume from saved model')
...
parser.add_argument('--result_dir', type=str, default='E:/6DPS/6D-CLGrasp/results\REAL', help='result directory')
parser.add_argument('--gpu', type=str, default='1', help='GPU to use')
opt = parser.parse_args()

# path for shape prior
mean_shapes = np.load('E:/6DPS/6D-CLGrasp/assets1/mean_points_emb.npy')

assert opt.data in ['val', 'real_test', 'Real_grasping']
if opt.data == 'val':
    result_dir = opt.result_dir
    file_path = 'CAMERA/val_list.txt'
    cam_fx, cam_fy, cam_cx, cam_cy = 577.6, 577.7, 319.8, 239.5
...
def detect():
    ...
    for path in tqdm(img_list):
        ...
        # print(img_path_parsing)
        # print(opt.data.split('_')[1])

        mrcnn_path = os.path.join('results/mrcnn_results', opt.data, 'results_{}_{}_{}.pkl'.format(
            img_path_parsing[2], img_path_parsing[3], img_path_parsing[4]))
...
def evaluate():
    ...
    # 分类结果
    class_ids = sorted(set([r['gt_class_ids'][0] for r in pred_results]))
    ...
    for class_id in class_ids:
        # 过滤该类别的预测结果
        class_pred_results = [r for r in pred_results if r['gt_class_ids'][0] == class_id]

        # 计算类别的mAP和Accuracy
        iou_aps_class, pose_aps_class, iou_acc_class, pose_acc_class = compute_mAP(class_pred_results, result_dir,
                                                                                   degree_thres_list, shift_thres_list,
                                                                                   iou_thres_list, iou_pose_thres=0.1,
        ...
        messages = []
        messages.append(f'Class {class_id} mAP:')
        messages.append('3D IoU at 25: {:.1f}'.format(iou_aps_class[-1, iou_25_idx] * 100))
        messages.append('3D IoU at 50: {:.1f}'.format(iou_aps_class[-1, iou_50_idx] * 100))
        messages.append('3D IoU at 75: {:.1f}'.format(iou_aps_class[-1, iou_75_idx] * 100))
        messages.append('5 degree, 2cm: {:.1f}'.format(pose_aps_class[-1, degree_05_idx, shift_02_idx] * 100))
        messages.append('5 degree, 5cm: {:.1f}'.format(pose_aps_class[-1, degree_05_idx, shift_05_idx] * 100))
        messages.append('10 degree, 2cm: {:.1f}'.format(pose_aps_class[-1, degree_10_idx, shift_02_idx] * 100))
        messages.append('10 degree, 5cm: {:.1f}'.format(pose_aps_class[-1, degree_10_idx, shift_05_idx] * 100))
        messages.append(f'Class {class_id} Acc:')
        messages.append('3D IoU at 25: {:.1f}'.format(iou_acc_class[-1, iou_25_idx] * 100))
        messages.append('3D IoU at 50: {:.1f}'.format(iou_acc_class[-1, iou_50_idx] * 100))
        messages.append('3D IoU at 75: {:.1f}'.format(iou_acc_class[-1, iou_75_idx] * 100))
        messages.append('5 degree, 2cm: {:.1f}'.format(pose_acc_class[-1, degree_05_idx, shift_02_idx] * 100))
        messages.append('5 degree, 5cm: {:.1f}'.format(pose_acc_class[-1, degree_05_idx, shift_05_idx] * 100))
        messages.append('10 degree, 2cm: {:.1f}'.format(pose_acc_class[-1, degree_10_idx, shift_02_idx] * 100))
        messages.append('10 degree, 5cm: {:.1f}'.format(pose_acc_class[-1, degree_10_idx, shift_05_idx] * 100))
