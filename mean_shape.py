...
parser.add_argument('--h5_file', type=str, default='data/obj_models/ShapeNetCore_4096.h5', help='h5py file')
parser.add_argument('--model', type=str, default='train_results/autoencoder/model_500.pth', help='resume model')
...
for i, data in enumerate(train_dataloader):
    ...
    batch_xyz = batch_xyz[:, :, :3].to(device)
...
mean_emb = np.empty((opt.n_cat, opt.emb_dim), dtype=np.float32)
catId_to_name = {0: 'bottle', 1: 'bowl', 2: 'camera', 3: 'can', 4: 'laptop', 5: 'mug'}
mean_points = np.empty((opt.n_cat, opt.n_pts, 3), dtype=np.float32)
for i in range(opt.n_cat):
    ...
    # 将NumPy数组转换为PyTorch张量，并移动到指定的设备（CPU或CUDA）
    assigned_emb = torch.tensor(mean[None, :], dtype=torch.float32, device=device)
