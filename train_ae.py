...
from auto_encoder import GSENet
# from lib.auto_encoder2 import GSENet2
...
#             batch_xyz, batch_label = data  # 从data提取xyz点云和对应的标签
#             batch_xyz = batch_xyz[:, :, :3].cuda() # batch_xyz进行切片操作，只取前三个通道（可能是x, y, z坐标）
#             embedding, point_cloud = estimator(batch_xyz) # 使用estimator模型对batch_xyz进行前向传播，并返回两个输出
#             loss, _, _ = criterion(point_cloud, batch_xyz)
#             val_loss += loss.item()
...
parser.add_argument('--max_epoch', type=int, default=500, help='max number of epochs to train')
parser.add_argument('--resume_model', type=str, default='', help='resume from saved model')
parser.add_argument('--result_dir', type=str, default='train_results/autoencoder',
