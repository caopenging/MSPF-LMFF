...
def estimateSimilarityTransform(source: np.array, target: np.array, verbose=False):
    ...
    for i in range(0, maxIter):
        ...
        # 内点判定条件
        PassThreshold = Scale * InlierT  # 根据缩放调整内点判定阈值
        ...
        if verbose:
            print(f"Iteration {i}: Inlier Ratio: {InlierRatio}, Best Inlier Ratio: {BestInlierRatio}")

        # 早停条件
        if (1 - (1 - BestInlierRatio ** 5) ** i) > confidence:
            if verbose:
                print(f"Stopping early at iteration {i} with inlier ratio: {BestInlierRatio}")
            break

    # 检查最终内点比率是否过低
    if BestInlierRatio < 0.1:
        print(f"[ WARN ] - Something is wrong. Small BestInlierRatio: {BestInlierRatio}")
