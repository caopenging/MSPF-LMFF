...
def uniform_sample(vertices, faces, n_samples, with_normal=False):
    ...
    faces = vertices[faces]  #将每个面的顶点坐标信息带入。
    vec_cross = np.cross(faces[:, 1, :] - faces[:, 0, :],  #计算三角面片的法向量
                         faces[:, 2, :] - faces[:, 0, :])  # 6014*3 每行包含一个面的法向量，每个法向量包含3个分量
    face_area = 0.5 * np.linalg.norm(vec_cross, axis=1)   #每个三角面片的面积 大小6014  沿第二维度表示每行，法向量维度
    cum_area = np.cumsum(face_area)  #表示 face_area 数组的累积和
    for i in range(n_samples):
        face_id = np.searchsorted(cum_area, np.random.random() * cum_area[-1])
        sampled_points[i] = random_point(faces[face_id, :, :])   #3072*3  随机采样的顶点
        normals[i] = vec_cross[face_id]                 #3072*3  法向量
    normals = normals / np.linalg.norm(normals, axis=1, keepdims=True)    #计算法向量长度并归一化为单位向量  2范数
    if with_normal:
...
def sample_points_from_mesh(path, n_pts, with_normal=False, fps=False, ratio=2):
    ...
    Args:
        path: path to OBJ file.  obj模型路径
        n_pts: int, number of points being sampled.  采样点数量1024
        with_normal: return points with normal, approximated by mesh triangle normal 返回有法线的点，用网格三角形法线近似
        fps: whether to use fps for post-processing, default False. 是否使用 fps 进行后期处理，默认为 False。
        ratio: int, if use fps, sample ratio*n_pts first, then use fps to sample final output.  如果使用 fps，则先对比率*n_pts 进行采样，然后使用 fps 对最终输出进行采样。
                3*1024
    ...
    """
    vertices, faces = load_obj(path)  # 顶点，面
    if fps:
        points = uniform_sample(vertices, faces, ratio*n_pts, with_normal)  #返回不带法线带顶点坐标信息的点 3072个  维度3072*3
        pts_idx = farthest_point_sampling(points[:, :3], n_pts)    #最远点采样的方法farthest_point_sampling，pts_idx表示存储的索引
        points = points[pts_idx]       #1024*3 维度，包含顶点xyz信息
    else:
        points = uniform_sample(vertices, faces, n_pts, with_normal)  #HDF5 点云维度4096*3 返回不带法线带顶点坐标信息的点
    return points
...
def transform_coordinates_3d(coordinates, sRT):
    ...
    denominator = np.where(new_coordinates[3, :] == 0, 1e-10, new_coordinates[3, :])
    new_coordinates = new_coordinates[:3, :] / denominator
    # new_coordinates = new_coordinates[:3, :] / new_coordinates[3, :]
    new_coordinates = np.nan_to_num(new_coordinates)
...
def compute_mAP(pred_results, out_dir, degree_thresholds=[180], shift_thresholds=[100],
    ...
    pkl_path = os.path.join(out_dir, 'E:/6DPS/6D-CLGrasp/results/learningrate0.001/mAP_Acc.pkl')
...
def plot_mAP(iou_aps, pose_aps, out_dir, iou_thres_list, degree_thres_list, shift_thres_list):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(os.path.join(out_dir, 'F:/mAP.png'),dpi=600)
    plt.close(fig)
    return
...
def draw_detections(img, out_dir, data_name, img_id, intrinsics, pred_sRT, pred_size, pred_class_ids,
    ...
    if draw_gt:
        for i in range(gt_sRT.shape[0]):
            if gt_class_ids[i] in [3]:
                sRT = align_rotation(gt_sRT[i, :, :])

                bbox_3d = get_3d_bbox(gt_size[i, :], 0)
                transformed_bbox_3d = transform_coordinates_3d(bbox_3d, sRT)
                projected_bbox = calculate_2d_projections(transformed_bbox_3d, intrinsics)
                img = draw_bboxes(img, projected_bbox, (255, 255, 255))
    ...
    for i in range(pred_sRT.shape[0]):
        ...
        transformed_bbox_3d = transform_coordinates_3d(bbox_3d, sRT)
        projected_bbox = calculate_2d_projections(transformed_bbox_3d, intrinsics)
        img = draw_bboxes(img, projected_bbox, 248, 75, 213)
